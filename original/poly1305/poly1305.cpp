...
{
	unsigned char data[] = { 0xf3, 0xf6 };//hash: 244, 198
	//char data[] = { 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17 };
	unsigned char key_char[] = { 0x85, 0x1f, 0xc4, 0x0c, 0x34, 0x67, 0xac, 0x0b, 0xe0, 0x5c, 0xc2, 0x04, 0x04, 0xf3, 0xf7, 0x00 };
	//unsigned char key_char[] = { 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16 };
	poly1305_state state;
	poly1305_init(&state);
	poly1305_key key;
	poly1305_setkey(&key, (const u8*)& key_char);
	poly1305_tail(&key, &state, (const u8*)& data, sizeof(data), false);
	le128 out;
	poly1305_emit(&state, &out, false);
...
void poly1305_setkey(struct poly1305_key* key, const u8* raw_key)
{
	...
	u32 temp = get_unaligned_le32(raw_key + 0);
	key->r[0] = (temp >> 0) & 0x3ffffff;
...
static void _poly1305(const struct poly1305_key* key, const u8* src,
...
static void _poly1305_generic(const struct poly1305_key* key, const u8* src,
